...
### T-031: インポート機能
- [ ] 「JSONファイルを読み込む」ボタンと `<input type="file">` を HomeView に設置
- [ ] ファイル読み込み → JSON パース → problems ストアに渡す
- [ ] インポート後に「○問追加、○問スキップ（重複）」をスナックバーで表示
- 完了条件: サンプルJSONをインポートして問題数が増える

### T-040: QuestionView（問題画面）
- [ ] session ストアから現在の問題を表示
- [ ] `type === "choice"` の場合: 4択ボタンを表示（順序ランダム）
- [ ] `type === "text"` の場合: テキスト入力フォームを表示
- [ ] 回答後に正誤フィードバック・正答・解説を表示
- [ ] 「次の問題」ボタンで次へ進み、全問終了後は ResultView へ遷移
