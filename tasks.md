...
- [x] スコア表示（例：「3 / 5 問正解」）
- [x] 不正解問題の一覧（問題文・正答・解説）
- [x] 「トップに戻る」ボタン
- 完了条件: セッション終了後に正しいスコアと不正解一覧が表示される ✅

### T-060: AnalysisView（苦手分析画面） ✅
- [x] 単元ごとの正答率テーブル（単元名・出題数・正解数・正答率）
- [x] 苦手問題一覧（正答率が低い順）
- 完了条件: 数回演習後に正答率が正しく反映される ✅

### T-070: ManageView（問題管理画面） ✅
- [x] インポート済み問題を一覧表示（ID・単元・問題文）
- [x] 単元でフィルタリング（ドロップダウン）
- [x] チェックボックスで複数選択 → 一括削除ボタン
- [x] 削除確認ダイアログ（誤操作防止）
...
### T-080: サンプルデータ作成 ✅
- [x] 数学（中学1〜3年）の問題を27問作成
- [x] 単元: 正の数・負の数 / 一次方程式 / 比例と反比例 / 連立方程式 / 一次関数 / 因数分解 / 二次方程式
- [x] `choice` と `text` の両 type を含める
- [x] ファイル名: `sample_questions.json`（リポジトリルートに配置、.gitignore 対象）
- 完了条件: サンプルJSONをインポートして全機能が動作する ✅

### T-090: レスポンシブ対応確認 ✅
- [x] DevTools でスマホ幅（375px）でレイアウトが崩れないことを確認
- [x] タップ操作で4択ボタンが押しやすいサイズになっていることを確認
